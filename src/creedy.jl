...
function pcreedy{T<:Real}(distmat::Matrix{T},ϵ::Real,minpts::Int)
  scan = dbscan(distmat,ϵ,minpts)
  asgn = scan.assignments
  dmsz = size(distmat)
  numCities = dmsz[1]
  ...
  ordDist = sortperm(vec(distmat))
  G = Graph(numCities)
  submats = Vector{Matrix{T}}(length(scan.seeds))
  for i in 1:length(scan.seeds)
    members = master[asgn .== i]
    submats[i] = distmat[members,members]
  end
  m_edges = pmap(subgreedy,submats)
  for i in 1:length(scan.seeds)
    members = master[asgn .== i]
    for (j,k) in m_edges[i]
      add_edge!(G,members[j],members[k])
    end
  end
  k = 1
