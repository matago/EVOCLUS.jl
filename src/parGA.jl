...
#
#
function parGA(objfun::Function,
             N::Int,
    ...
    fitness = SharedArray(Float64,populationSize)
    trueFit = SharedArray(Float64,populationSize)
    population = SharedArray(Int, (N,populationSize))
    offspring = SharedArray(Int, (N,populationSize))

    # Generate population
    @parallel for i in 1:populationSize
    ...
    while true
        # Select offspring
        selected = selection(fitness.s, populationSize)

        # Perform mating
        offidx = randperm(populationSize)
        @parallel for i in 1:2:populationSize
